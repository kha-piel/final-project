...
create or replace function public.resolve_login_email(login_identity text)
returns citext
language plpgsql
security definer
set search_path = public
as $$
declare
    normalized_identity citext;
    resolved_email citext;
begin
    normalized_identity := nullif(trim(login_identity), '');
    ...
    where up.status = 'active'
      and (
        up.email = normalized_identity
        or up.username = normalized_identity
      )
    limit 1;
